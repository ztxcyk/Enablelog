...
BOOL CRegistry::KeyExists(CString strKey, HKEY hRootKey)
{
	...
	LONG lResult = ::RegOpenKeyEx(hRootKey, LPCTSTR(strKey), 0,
		KEY_READ, &hKey);        //2007/06/29 by yxf
	::RegCloseKey(hKey);
	if (lResult == ERROR_SUCCESS) return TRUE;
	return FALSE;
}
...
//得到某一主键的子键
DWORD CRegistry::EnumSubKey(CStringArray *SubArray,LPCSTR pKeyName)
{
	...
	{
		if( (dwRtn=RegOpenKeyEx(m_hRootKey,pKeyName,
            0, KEY_ENUMERATE_SUB_KEYS , &hKey )) == ERROR_SUCCESS)
		{
            ...
            while (dwRtn == ERROR_SUCCESS )
            {
				...
				{
					CString tempstr=szSubKey;	
					SubArray->Add(tempstr);	
					index++;
				}
            }
            RegCloseKey(hKey);
...
BOOL CRegistry::WriteBinary(CString strName,  BYTE* pbtValue, int nLen)
{
	ASSERT(m_strCurrentPath.GetLength() > 0);
	BOOL bSuccess = TRUE;
	HKEY hKey;
...
DWORD CRegistry::DeleteKeyNT( LPTSTR pKeyName )
{
      DWORD   dwRtn, dwSubKeyLength;
      LPTSTR  pSubKey = NULL;
      TCHAR   szSubKey[MAX_PATH]; // (256) this should be dynamic.
      ...
      if ( pKeyName &&  lstrlen(pKeyName))
      {
            ...
            0, KEY_ENUMERATE_SUB_KEYS | DELETE, &hKey )) == ERROR_SUCCESS)
         {
            while (dwRtn == ERROR_SUCCESS )
            {
               ...
               if(dwRtn == ERROR_NO_MORE_ITEMS)
               {
                  dwRtn = RegDeleteKey(m_hRootKey, pKeyName);
                  break;
               }
               else if(dwRtn == ERROR_SUCCESS)
			   {
				   CString tempstr=CString(pKeyName)+"\\"+szSubKey;
                   dwRtn=DeleteKeyNT(tempstr.GetBuffer(0));
...
BOOL CRegistry::ReadBinary(CString strName, BYTE* pBinArray,ULONG nLen)
{
	DWORD dwType = REG_BINARY;
	HKEY hKey;
	BOOL bSuccess = TRUE;
	...
	if (::RegQueryValueEx(hKey, LPCTSTR(strName), NULL,
		&dwType, pBinArray, &nLen) != ERROR_SUCCESS) bSuccess = FALSE;
...
BOOL CRegistry::WriteMSZString(CString strName, CString strValue)
{
	...
	char  temp[200];
	memcpy(temp, strValue.GetBuffer(0), strValue.GetLength()+1);
	temp[strValue.GetLength()+1] = 0;
	return RegSetValueEx(hKey,LPCTSTR(strName),0L,REG_MULTI_SZ,
		(CONST  BYTE* )temp,strValue.GetLength()+2);
}

//ExpandSZ比MULSZ未尾少0000

BOOL CRegistry::WriteExpandString(CString strName, CString strValue)
{
	...
	memcpy(temp, strValue.GetBuffer(0), strValue.GetLength()+1);
	temp[strValue.GetLength()+1] = 0;
	return RegSetValueEx(hKey,LPCTSTR(strName),0L,REG_EXPAND_SZ,
		(CONST  BYTE* )temp,strValue.GetLength());
}

BOOL CRegistry::WriteMSZString(CString strName, char* pbtValue,int buflen)
{
	...
	return RegSetValueEx(hKey,LPCTSTR(strName),0L,REG_MULTI_SZ,
		(CONST  BYTE* )pbtValue,len);
}

BOOL CRegistry::ReadMSZString(CString strName, char* pbtValue)
{
	...
	memcpy(pbtValue,sz,dwSize);
	return TRUE;
}

//不放到缓冲
void CRegistry::FlushWhenWrite(BOOL flush)
{
   m_bLazyWrite=!flush;
}

void CRegistry::FlushAll()
{
   ::RegFlushKey(HKEY_CLASSES_ROOT);  //这句执行有问题，实际和下面一句一样效果
